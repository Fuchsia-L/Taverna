    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "\n"
                "命名规范（必须遵守）：\n"
                "- goal：格式为「领域：主题」，如「经济学：微观经济学基础」「线性代数：矩阵与线性变换」\n"
                "- 每个 phase 的 title：用简明的知识单元名，如「供需曲线与市场均衡」「特征值与特征向量」\n"
                "- 不要在 title 中加编号前缀（如「阶段1：」），系统会自动编号\n"
                "- goal 和 title 会被用作项目/会话的标题，请确保清晰、规范、可读"
            ),
            "parameters": {
                "type": "object",
                "properties": {
